...
greift für Elektronen das \name{Pauli}-Prinzip, welches besagt, dass es
...
Nullpunkt haben kann, wird als \name{Fermi}sche Grenzenergie $\zeta$
bezeichnet.
...
\begin{figure}[]
\centering
\includegraphics[width=0.8\textwidth]{diode}
...
\begin{figure}[]
\centering
\includegraphics[width=0.8\textwidth]{kennlinie}
...
%
Da die Elektronen im elektrischen Feld der Diode gleichmäßig beschleunigt werden und innerhalb der Diode nicht abgebremst werden, gilt hierbei nicht das \name{Ohm}sche Gesetz, sodass sich auch kein linearer Zusammenhang zwischen Spannung und Stromstärke einstellt.
...
Das Ergebnis dieser Rechnung wird als \name{Langmuir}-\name{Schottky}sche Raumladungsgleichung bezeichnet und ist in Formel~\eqref{eq:raumladung} wiedergegeben. Dabei bezeichnet $j$ die Stromdichte, $V$ die angelegte Spannung und $a$ den Abstand zwischen Kathode und Anode.
\begin{equation}
...
Um eine Formel für die Anzahl der austretenden Elektronen pro Zeit und Fläche aus der Kathode, also derSättigungsstromdichte $j_S$, zu erhalten, wird die \name{Fermi}-\name{Dirac}-Verteilung~\eqref{eq:fermi_dirac} für große Energien der Elektronen betrachten.
Anschließend werden alle Elektronen pro Zeit und Fläche gezählt, die eine eine Geschwindigkeitskomponente senkrecht zur Metalloberfläche besitzen, welche ausreicht, um das Metall zu verlassen. Multipliziert man die erhaltene Anzahl mit der Elementarladung $e_0$, ergibt sich die gesuchte Formel, welche \name{Richardson}-Gleichung genannt wird und in Formel~\eqref{eq:richard} angegeben ist. $T$ ist die Temperatur, $k$ die \name{Boltzmann}-Konstante und $\phi$ das Austrittspotential.
%
\begin{equation}
